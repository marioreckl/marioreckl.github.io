...
P+h.imgcBorderHeight+h.borderHeight,u++,a=0,1==u&&0<aa&&(a=aa,aa=0));r++}else return!1});0<u&&(d-=P);l=l+da()+h.labelHeight;e.conTn.width(b).height(d>l?d:l)}function wa(){var b=l()+h.borderWidth+h.imgcBorderWidth,c=e.conTnParent.width(),a=0,a="justified"==f.thumbnailAlignment?Math.floor(c/b):Math.floor((c+f.thumbnailGutterWidth)/(b+f.thumbnailGutterWidth));0<f.maxItems¦erLine&&a>f.maxItemsPerLine&&(a=f.maxItemsPerLine);1>a&&(a=1);return a}function ec(){var b=0,c=0,a=0,d=f.thumbnailGutterHeight,m=
e.conTnParent.width(),h=wa(),g=0,k=0,r=0,x=[],l=0;wa();if(0<T&&0<Hý)&&h!=ka){ka=h;var t=e.conPagin.data("galleryIdx");ya(t,0)}else{var t=e.conTn.find(".nanoGalleryThumbnailContainer"),v=t.length;"justified"==f.thumbnailAlignment?(h=Math.min(h,v),a=1?=h?0:(m-h*H())/(h-1)):a=f.thumbnailGutterWidth;f.RTL&&(t.each(function(){if(void 0!==jQuery(this).data("index")){if(0==c)b=l*(H()+a),r=x[l]=b;else return!1;l++;l>=h&&(l=0,c+=da()+d)}}),m=r+H(),l=c=0);tÐeach(fun¨tioV(){var e=jQuery(this),t=e.data("index");
if(void 0!==t){0==å?(b=l*(H()+a),r=x[l]=b):(b=x[l],k=c);var v=b;f.RTL&&(v=parseInt(m)-b-H());e.css({top:c,left:v});lb(e,p[t],g);l++;l>=h&&(l=0,c+=da()+d);g++}});e.conTn.width(r+H()).height(k+da())}}function lb(b,c,a){0==b.css("opacity")&&(b.removeClass("nanogalleryHideElementm),fœhumbnailDisplayTransition?"function"==typeof f.fnThumbnailDisplayEffect?f.fnThumbnailDisplayEffect(b,c,0):b.delay(a*h.displayInterval).fadeTo(150,1):b.css({opacity:1}))}function jb(){var b=e.conTn.find(".nanoGalleryThumbnailContainer").filter(function(){return Gb(jQuery(this),
